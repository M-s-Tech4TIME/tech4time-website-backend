...
| `check_password_fields.py` | every `type="password"` input carries a show/hide switch naming **its own** id. A field with no switch is **silent** — it simply works the way fields worked before, and the only way to notice is for somebody to look for a control they expected and not find it. There are twelve across `public/login.php`, `setup.php`, `reset.php` and `sections/account.php`, and three of those are pages a person meets once or twice; a hand count missed the seventh on the account screen. The id is **matched, not counted** — a wrapper holding a switch that names a different field renders perfectly and reveals the wrong box, or nothing, and counting would pass it |
...
| `test_privacy_admin.py` | the privacy editor: sections with Markdown bodies — add, remove and **hide** of sections and summary points, that hostile input round-trips byte-identical (never sanitised), that preview renders without saving (revision on disk unmoved), that a section rendering to nothing is refused, that a section which has been named keeps its anchor when one with the same heading is added above it, and that a policy disagreeing with the contact page **still saves** |
| `test_legal_admin.py` | the legal hub: three rows listed, planned documents linked nowhere, show/hide writing + publishing with every word kept, and refusals for documents with no editor |
...
| `test_company_admin.py` | the company profile editor: its five lists, and add, remove, **hide** and reorder on each — plus the rule that hiding is not deleting, and that a picture may only point inside this site. And one check that is not about this form at all: that saving it leaves the deprecated `milestones` band **exactly as it found it**. That band is named in `COMPANY_BANDS` and `COMPANY_LISTS` and rendered nowhere, which is the shape that blanks a band on every save; `COMPANY_MOVED_BANDS` is what stops it, and nothing else would notice if it were removed |
| `test_milestones_admin.py` | the milestones editor: the timeline, add, remove, **hide** and reorder, the year rule (`2024` or `2024–2025`, and a refusal with the entry named), and above all the **read-through** — that the form comes up holding the company document's entries and heading before anything has been saved, that the first save writes them here and leaves that document untouched, and that emptying the list afterwards leaves it empty. Keying the fallback on an empty list rather than on the revision would hand every entry straight back, which is the editor refusing to do what it was told |
| `test_upload.py` | `lib/upload.php`, and mostly not by asking what it refused: it checks what came OUT still carrying what went in. EXIF stripped, an appended payload gone, an oversized picture reduced. Also **which pictures count as in use** — every name in `CONTRACT_DOCUMENTS` must answer `contract_images()`, an office photograph must be among them, which it was not, and a ladder's rungs must be among them in each of the thirteen seats a document can hold a picture in. And **which widths get stored**: slot-and-source cases against the measured table, the alpha channel read back off every rung, and the two slot lists compared in both directions so a misspelt slot cannot pass as a picture that simply does not ladder. Those cases sit **before** the GD gate on purpose: nothing in them decodes a picture, so gating them would mean the check that caught a real deletion bug never ran on a machine without the extension. Skips the re-encoding cases with a notice where PHP has no GD; CI installs `php-gd` so they always run there. **It restores `public/uploads/` by contents, not by name** — it deletes that directory outright to prove `upload_store()` recreates it, which also destroyed the committed `.gitignore`, and a cleanup that only removed unrecognised files could not put that back |
...
| `test_md_toolbar.py` | the Markdown ribbons on the legal editor, pressed in a real browser: bold nesting, notes and tables standing on their own lines, whole-line lists, links taking the prompt's answer — prompts stubbed, because what is under test is what the toolbar does with the answer — and the field still saving afterwards |
