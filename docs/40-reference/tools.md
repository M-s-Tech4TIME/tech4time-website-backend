...
| `test_settings_admin.py` | the site-identity editor: the rail row and the Overview tile, the index and its four part screens, an unknown part landing on the index rather than raising, and the two standing notices — an empty dark logo half, and a replaced logo with the shipped tab icon still under it — each appearing on its **own** condition and neither drawn as an error. Plus the road: `settings_edit()` writes, publishes, and **merges** rather than rebuilding, proved against a value in a part the save does not touch |
